  ...
  (
    ...
    const updateMouseCoords = useCallback(() => {
      ...
      if (cursor.current && typeof onCursorMove === "function") {
        onCursorMove(
          context.transformedPoint(cursor.current.x, cursor.current.y)
        );
      }
    }, [onCursorMove]);
      ...
      const getChildren = (child: React.ReactNode) => {
        if (Array.isArray(child)) {
          ...
          if (asElement) {
            if (
              asElement.props &&
              asElement.props.children &&
              asElement.type !== Tooltip
            ) {
    ...
    markers.current = flatChildren.filter(
      (child) => child.type && child.type === Marker
    );
    const getMarkerChild = (key: string): React.ReactElement | undefined =>
      markers.current.find((child) => child && child.props.markerKey === key);
    dropZones.current = flatChildren.filter(
      (child) => child.type && child.type === DropZone
    );
    ...
    const getDropZoneTouchingCursor = useCallback(():
      | React.ReactElement
      | undefined => {
   ...
   /*  useImperativeHandle(ref, () => ({
    ...
    const updateContainmentScale = () => {
      ...
      if (imgWidth && imgHeight) {
        const widthScaledHeight =
          (imgHeight / imgWidth) * canvasRef.current.width;
        let updatedScale = 1;
    ...
    const handleImageLoad = useMemo(
      () => () => {
        ...
        if (imgWidth && imgHeight) {
          ...
          const widthScaledHeight =
            (imgHeight / imgWidth) * canvasRef.current.width;
          const heightScaledWidth =
            (imgWidth / imgHeight) * canvasRef.current.height;
    ...
    const resize = useCallback(() => {
      ...
      if (cursor.current !== null) {
        const cursorXProportion =
          cursor.current.x / canvasRef.current.clientWidth;
        const cursorYProportion =
          cursor.current.y / canvasRef.current.clientHeight;
